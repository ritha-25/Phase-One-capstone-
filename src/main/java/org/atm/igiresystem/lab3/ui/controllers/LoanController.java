...
public class LoanController implements Initializable {
    ...
    private void handleRequestLoan() {
        ...
        if (idx < 0) {
            showMsg("Please select an account to receive the loan.", false);
            return;
        }

        String amountText = amountField.getText().trim();
        if (amountText.isEmpty()) {
            showMsg("Please enter a loan amount.", false);
            return;
        }
        ...
        try {
            ...
            showMsg("Invalid amount. Please enter a number.", false);
            return;
        }

        String purpose = purposeField.getText().trim();
        if (purpose.isEmpty()) {
            showMsg("Please describe the purpose of this loan.", false);
            return;
        }

        String pin = pinField.getText().trim();
        if (pin.isEmpty()) {
            showMsg("Please enter your PIN to confirm.", false);
            return;
        }

        if (!accountService.validatePinByCustomer(Session.getCustomer().getId(), pin)) {
            showMsg("Incorrect PIN. Please try again.", false);
            return;
        }
    ...
    public void loadMyLoans() {
        ...
        for (Loan loan : loans) {
            ...
            amountLabel.setStyle("-fx-font-size:16px;-fx-font-weight:bold;-fx-text-fill:#003087;");
            javafx.scene.layout.HBox.setHgrow(amountLabel, javafx.scene.layout.Priority.ALWAYS);

            String statusColor = "APPROVED".equals(loan.getStatus()) ? "#003087"
                               : "REJECTED".equals(loan.getStatus()) ? "#DC2626" : "#D97706";
            ...
            purposeLabel.setStyle("-fx-font-size:12px;-fx-text-fill:#6B7280;");

            double repayable = loan.getTotalRepayable();
            Label repayLabel = new Label("Total repayable: " + String.format("%.2f", repayable) + " RWF  (10% interest)");
            repayLabel.setStyle("-fx-font-size:11px;-fx-text-fill:#9CA3AF;");

            String dateStr = loan.getRequestedAt() != null
